...
export default function CreateUserModal({ onClose, onSuccess }: CreateUserModalProps) {
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className='fixed top-1/2 left-1/2 -translate-1/2 bg-black/90 border-2 border-border rounded-3xl px-4 py-5 flex flex-col gap-4 w-lg max-w-[calc(100%-1rem)] z-10 max-h-[90vh] overflow-y-auto'
        >
                ...
                {password.length > 0 && (
                    <div className='bg-muted border-2 border-border rounded-lg px-3 py-2 flex flex-col gap-1'>
                        ...
                        <ul className='flex flex-col gap-0.5'>
                            <li className={`text-xs flex items-center gap-2 transition-colors ${reqs.length ? "text-blue-400 font-medium" : "text-muted-foreground/70"}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${reqs.length ? "bg-blue-400" : "bg-muted"}`}></span>
                                At least 12 characters
                            </li>
                            <li className={`text-xs flex items-center gap-2 transition-colors ${reqs.uppercase ? "text-blue-400 font-medium" : "text-muted-foreground/70"}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${reqs.uppercase ? "bg-blue-400" : "bg-muted"}`}></span>
                                One uppercase letter
                            </li>
                            <li className={`text-xs flex items-center gap-2 transition-colors ${reqs.lowercase ? "text-blue-400 font-medium" : "text-muted-foreground/70"}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${reqs.lowercase ? "bg-blue-400" : "bg-muted"}`}></span>
                                One lowercase letter
                            </li>
                            <li className={`text-xs flex items-center gap-2 transition-colors ${reqs.number ? "text-blue-400 font-medium" : "text-muted-foreground/70"}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${reqs.number ? "bg-blue-400" : "bg-muted"}`}></span>
                                One number
                            </li>
                            <li className={`text-xs flex items-center gap-2 transition-colors ${reqs.special ? "text-blue-400 font-medium" : "text-muted-foreground/70"}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${reqs.special ? "bg-blue-400" : "bg-muted"}`}></span>
            ...
            <div className='flex flex-row gap-2 justify-end mt-2'>
                ...
                <button
                    title='Create User'
                    type='button'
                    className='bg-green-400/20 px-4 py-1 font-semibold border-2 border-border rounded-lg cursor-pointer transition-colors hover:bg-green-400/30 disabled:opacity-50 disabled:cursor-not-allowed'
