...
interface CancelDeductionModalProps {
    deductionId: string
    onClose: () => void
    onConfirm: (deductionId: string, reason?: string) => Promise<void>
}

export function CancelDeductionModal({
    deductionId,
    onClose,
    onConfirm,
}: CancelDeductionModalProps) {
    const [reason, setReason] = useState("")
    const [loading, setLoading] = useState(false)

    const handleConfirm = async () => {
        setLoading(true)
        await onConfirm(deductionId, reason || undefined)
        setLoading(false)
    }
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm'>
            ...
            >
                <div className='p-6 border-b border-white/10'>
                    <h3 className='text-xl font-bold text-red-400'>Cancel Deduction</h3>
